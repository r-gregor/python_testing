...
for el in camp_L:
	print("{0:<13}{1:>12.2f}{2:>12.2f}".format(el[0], c_nsez[el[0]], c_vsez[el[0]]))
print()

print()
print("{0:<13}{1:>12}{2:>12}".format('Ime', 'dni_nsez', 'dni_vsez'))
print("-"*37)
for nms in ['Gregor', 'Tadeja', 'Zala', 'Mark', 'Spela', 'Camp']:
...
skupaj_odr = (st_dni_odr_nsez * c_nsez['Odrasli']) + (st_dni_odr_vsez * c_vsez['Otroci'])
skupaj_otr = (st_dni_otr_nsez * c_nsez['Otroci']) + (st_dni_otr_vsez * c_vsez['Otroci'])
skupaj_parc = (d_nsez['Camp'] * c_nsez['Parcela']) + (d_vsez['Camp'] * c_vsez['Parcela'])
skupaj_avto = (d_nsez['Camp'] * c_nsez['Avto']) + (d_vsez['Camp'] * c_vsez['Avto'])
skupaj_el = (d_nsez['Camp'] * c_nsez['Elektrika']) + (d_vsez['Camp'] * c_vsez['Elektrika'])
