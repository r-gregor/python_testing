...
for el in ['odr', 'otr', 'parc', 'avto', 'el']:
	print("{0:<13}{1:>12.2f}{2:>12.2f}".format('camp_'+el, c_nsez['camp_'+el+'_nsez'], c_vsez['camp_'+el+'_vsez']))
print()
...
for nms in ['Gregor', 'Tadeja', 'Zala', 'Mark', 'Spela', 'parc_avt_el']:
	print("{0:<13}{1:>12}{2:>12}".format(nms, d_nsez[nms + '_dni_nsez'], d_vsez[nms + '_dni_vsez']))
print()
...
# CAMP -------------------------------------------------------------------------------------------------------
st_dni_odr_nsez = d_nsez['Gregor_dni_nsez'] + d_nsez['Tadeja_dni_nsez'] + d_nsez['Zala_dni_nsez'] + d_nsez['Mark_dni_nsez']
st_dni_odr_vsez = d_vsez['Gregor_dni_vsez'] + d_vsez['Tadeja_dni_vsez'] + d_vsez['Zala_dni_vsez'] + d_vsez['Mark_dni_vsez']
st_dni_otr_nsez = d_nsez['Spela_dni_nsez']
st_dni_otr_vsez = d_vsez['Spela_dni_vsez']

skupaj_odr = (st_dni_odr_nsez * c_nsez['camp_odr_nsez']) + (st_dni_odr_vsez * c_vsez['camp_otr_vsez'])
skupaj_otr = (st_dni_otr_nsez * c_nsez['camp_otr_nsez']) + (st_dni_otr_vsez * c_vsez['camp_otr_vsez'])
skupaj_parc = (d_nsez['parc_avt_el_dni_nsez'] * c_nsez['camp_parc_nsez']) + (d_vsez['parc_avt_el_dni_vsez'] * c_vsez['camp_parc_vsez'])
skupaj_avto = (d_nsez['parc_avt_el_dni_nsez'] * c_nsez['camp_avto_nsez']) + (d_vsez['parc_avt_el_dni_vsez'] * c_vsez['camp_avto_vsez'])
skupaj_el = (d_nsez['parc_avt_el_dni_nsez'] * c_nsez['camp_el_nsez']) + (d_vsez['parc_avt_el_dni_vsez'] * c_vsez['camp_el_vsez'])
